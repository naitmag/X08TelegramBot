...
from handlers import manage_cabs, show_author, weather_request, \
    random_request, send_user_id, wrong_chat_type, start_greetings, send_contacts, \
...
def main():
    ...
    bot.register_message_handler(read_database, commands=['read'])
    bot.register_message_handler(clear_database, commands=['drop'])
    ...
    bot.register_message_handler(send_user_id, commands=['id'])
